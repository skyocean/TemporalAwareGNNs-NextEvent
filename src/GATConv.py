...
class DualGATModel(nn.Module):
    def __init__(self, num_event_features, num_embedding_features, embedding_dims, 
                 ...
                 output_dim, num_heads, edge_dim=1):
        super(DualGATModel, self).__init__()

        # Embedding for core event IDs
        self.embedding = nn.Embedding(num_embeddings=num_embedding_features, embedding_dim=embedding_dims)

        # GAT layers for embedding path
        self.gat_embed = GATConv(embedding_dims, gat_hidden_dim_embed, heads=num_heads, concat=True)

        # GAT layers for event feature path
        self.gat_event = GATConv(num_event_features, gat_hidden_dim_event, heads=num_heads, concat=True)

        # GAT layer for concatenated embeddings
        concat_input_dim = (gat_hidden_dim_embed + gat_hidden_dim_event) * num_heads
        self.gat_concat = GATConv(concat_input_dim, gat_hidden_dim_concat, heads=num_heads, concat=True)
    ...
    def forward(self, data_event):
        ...
        # ----- Embedding path -----
        x_embed = self.embedding(data_event.event_ids.view(-1))  # Shape: (num_nodes, embedding_dim)
        x_embed = self.gat_embed(x_embed, data_event.edge_index, edge_attr=edge_attr)

        # ----- Event feature path -----
        x_event = data_event.x
        x_event = self.gat_event(x_event, data_event.edge_index, edge_attr=edge_attr)
