...
def prepare_data_core_2edges(event_encode, core_encode, scaled_time_diffs, edge_types_encoded, node_times):
    data_list_event = []
        ...
        for event_data, labels in loader:
            ...
            if return_attention:
                ...
                for graph_idx in range(num_graphs):
                    ...
                    etype_score_embed = normalize_edge_scores(attn_data['edge_type_score_embed'], edge_indices) if 'edge_type_score_embed' in attn_data else None
                    etype_score_event = normalize_edge_scores(attn_data['edge_type_score_event'], edge_indices) if 'edge_type_score_event' in attn_data else None
                    etype_score_final = normalize_edge_scores(attn_data['edge_type_score_final'], edge_indices) if 'edge_type_score_final' in attn_data else None
                    ...
                    graph_attn = {
                        ...
                        'edge_type_score_embed': etype_score_embed,
                        'edge_type_score_event': etype_score_event,
                        'edge_type_score_final': etype_score_final,
...
class TimeAwareETGATConv(GATConv):
    def __init__(self, in_channels, out_channels, heads=1, concat=True, lambda_decay=0.1, edge_type_dim=None, **kwargs):
        super(TimeAwareETGATConv, self).__init__(in_channels, out_channels, heads=heads, concat=concat, **kwargs)
        ...
        if edge_type_dim is None:
            raise ValueError("edge_type_dim must be specified for TimeAwareGATConv.")
    ...
    def edge_attention(self, x_i, x_j, time, edge_attr, edge_index):
        ...
        if edge_attr is not None:
            ...
            edge_type_score = torch.einsum('ed,hd->eh', edge_attr, self.edge_type_att)  # [E, H]
            alpha = alpha + edge_type_score

            # Compute decay from time
            delta_t = time
            decay = torch.exp(-self.lambda_decay * delta_t).unsqueeze(-1)  # [E, 1]
            alpha = alpha * decay 
    ...
    def message(self, x_i, x_j, time, edge_attr, edge_index, index, ptr, size_i):
        #print("edge_attr inside message:", edge_attr.shape)
    ...
    def forward(self, x, edge_index, edge_attr=None, time=None, return_attention=False):
        ...
        out = self.propagate(x=x, edge_index=edge_index, edge_attr=edge_attr, time=time, size=None)
...
class DualGATTimeAwareETModel(nn.Module):
    def __init__(self, 
                 ...
                 edge_type_dim, 
                 lambda_decay):
        super(DualGATTimeAwareETModel, self).__init__()
        ...
        self.gat_embed = TimeAwareETGATConv(embedding_dims, gat_hidden_dim_embed, heads=num_heads,
                                 concat=True, edge_dim=edge_attr_dim, lambda_decay=lambda_decay, edge_type_dim=edge_type_dim)

        # GAT layers for raw event feature path
        self.gat_event = TimeAwareETGATConv(num_event_features, gat_hidden_dim_event, heads=num_heads,
                                 concat=True, edge_dim=edge_attr_dim, lambda_decay=lambda_decay, edge_type_dim=edge_type_dim)

        # GAT for combined path
        concat_input_dim = (gat_hidden_dim_embed + gat_hidden_dim_event) * num_heads
        self.gat_concat = TimeAwareETGATConv(concat_input_dim, gat_hidden_dim_concat, heads=num_heads,
                                  concat=True, edge_dim=edge_attr_dim, lambda_decay=lambda_decay, edge_type_dim=edge_type_dim)
    ...
    def forward(self, data_event, return_attention=False):
        ...
        #print("raw edge_type:", data_event.edge_type.shape, data_event.edge_type.dtype)
        edge_type = data_event.edge_type
        edge_attr = self.edge_type_emb(edge_type)
        #print("embedded edge_attr:", edge_attr.shape)
        #time = normalize_time_01(data_event.time, data_event.batch)
        ...
        if return_attention:
            x_embed, attn_embed = self.gat_embed(x_embed, edge_index=edge_index, edge_attr=edge_attr, time=time, return_attention=True)
        else:
            x_embed = self.gat_embed(x_embed, edge_index=edge_index, edge_attr=edge_attr, time=time)

        # ---- Event feature path ----
        if return_attention:
            x_event, attn_event = self.gat_event(data_event.x, edge_index=edge_index, edge_attr=edge_attr, time=time, return_attention=True)
        else:
            x_event = self.gat_event(data_event.x, edge_index=edge_index, time=time, edge_attr=edge_attr)

        # ---- Concatenation ----
        x = torch.cat([x_embed, x_event], dim=1)
        if return_attention:
            x, attn_final = self.gat_concat(x, edge_index=edge_index, edge_attr=edge_attr, time=time, return_attention=True)
        else:
            x = self.gat_concat(x, edge_index=edge_index, edge_attr=edge_attr, time=time)
        ...
        if return_attention:
            return out, {
                ...
                'edge_type_score_embed': self.gat_embed._edge_type_score.detach().cpu() if self.gat_embed._edge_type_score is not None else None,
                'edge_type_score_event': self.gat_event._edge_type_score.detach().cpu() if self.gat_event._edge_type_score is not None else None,
                'edge_type_score_final': self.gat_concat._edge_type_score.detach().cpu() if self.gat_concat._edge_type_score is not None else None,
            }
        return out
...
def plot_edge_attention_correlation(attention_data, length_ranges, figsize=(16, 12), save_path=None):
    """
    Q1 Journal-Ready Edge-Attention Relationship Plot (Fixed Version)
    """
    # Prepare dataframe
    records = []
    for attn in attention_data:
        seq_len = attn['edge_index'].shape[1]
        length_group = next((lr[2] for lr in length_ranges 
                          if lr[0] <= seq_len < lr[1]), None)
        if not length_group:
            continue
    ...
    # Hexbin density contours
    hb = ax.hexbin(
        df['edge_score'], df['attention'], 
        gridsize=30, cmap='Greys', 
        mincnt=1, alpha=0.2, linewidths=0
    )
    
    # Scatter plot with proper palette handling
    sns.scatterplot(
        data=df.sample(frac=0.3, random_state=42),
        x='edge_score', y='attention',
        hue='length_group', hue_order=length_order,
        palette=palette, alpha=0.7, edgecolor='none',
        size='seq_length', sizes=(20, 100),
        ax=ax
    )
    ...
    sns.kdeplot(
        data=df, x='edge_score',
        hue='length_group', hue_order=length_order,
        palette=palette, fill=True, alpha=0.2,
        linewidth=0.5, ax=ax_histx,
        legend=False
    )
    ax_histx.set_xlabel('')
    ...
    for i, edge_type in enumerate(top_edge_types):
        ...
        if len(edge_df) > 10:
            r_edge, p_edge = stats.pearsonr(edge_df['edge_score'], edge_df['attention'])
            ax.text(
                0.05, 0.85 - 0.05*i,
    ...
    #ax.set_title('Edge-Attention Relationship by Sequence Length', fontsize=14, pad=20)
    
    # Legend for length ranges (every 3rd for space)
    ...
    #plt.tight_layout()
    fig.suptitle(
        'Edge-Attention Relationship by Sequence Length',
        fontsize=14, y=0.92, ha='center')
    fig.text(
        0.5, 0.89,
        '(Top) Edge score distribution by length range; '
        '(Middle) Scatter plot with length-stratified trendlines; '
        '(Right) Attention score distribution',
...
def plot_importance_timeline_edge(attention_data, min_length=100, max_length=None, figsize=(20, 8), save_path=None):
   ...
   # Find first sequence meeting length criteria
    ...
    # Edge type scores (NEW)
    edge_embed = long_seq['edge_type_score_embed'].cpu().numpy()
    edge_event = long_seq['edge_type_score_event'].cpu().numpy()
    edge_final = long_seq['edge_type_score_final'].cpu().numpy()
    ...
    time_diffs = np.diff(time)
    avg_time_diff = np.mean(time_diffs) if len(time_diffs) > 0 else 0.02
    bar_width = avg_time_diff * 0.8  # Slightly narrower for more bars
    edge_offset = -bar_width * 0.6    # Edge bars left of center
    alpha_offset = bar_width * 0.6    # Attention bars right of center
    ...
    # 2. Edge score bars (NEW - behind attention bars)
    for i, t in enumerate(time):
        # Edge embed score (bottom layer)
        ax.bar(t+edge_offset, edge_embed[i], width=bar_width*0.9, color='#aec7e8', alpha=0.9,
              label='Edge Embed Score' if i == 0 else "")
        # Edge event score (middle layer)
        ax.bar(t+edge_offset, edge_event[i], width=bar_width*0.9, color='#ffbb78', alpha=0.9,
              bottom=edge_embed[i], label='Edge Event Score' if i == 0 else "")
    
    # 3. Attention bars
    for i, t in enumerate(time):
        ax.bar(t+alpha_offset, alpha_embed[i], width=bar_width*0.9, color='#17becf', alpha=0.7,
              label='Embed Attention' if i == 0 else "")
        ax.bar(t+alpha_offset, alpha_event[i], width=bar_width*0.9, color='#e377c2', alpha=0.7,
    ...
    plt.plot(time, edge_final, label='Edge Final Score', color='#8c564b', linestyle='-.', linewidth=2)  # NEW
